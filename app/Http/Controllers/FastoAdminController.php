...
class FastoAdminController extends Controller
{
    ...
    public function dashboard()
    {
        ...
        $organisations = Organisation::where('user_id', auth()->id())->latest()->count();
        $contacts = Client::where('user_id', auth()->id())->latest()->count();
        $leads = Lead::where('user_id', auth()->id())->latest()->count();
        $deals = Deal::where('user_id', auth()->id())->latest()->count();

        $monthlyCounts = Client::where('user_id', auth()->id())
        ...
        ksort($monthlyCounts);
        $countsClientsArray = array_values($monthlyCounts);
        return view('fasto.dashboard.index', compact(
            'page_title',
            'page_description',
            'organisations',
            'contacts',
    ...
    public function page_login()
    {
        ...
        $page_description = 'Some description for the page';
        return view('fasto.page.login', compact('page_title', 'page_description'));
    }
